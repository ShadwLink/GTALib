...
import _3DModel.model.Polygon;
import _3DModel.model.Strip;
import _3DModel.model.Vertex;
...
public class NewConvert{
    ...
    public void createSystemBlock(){
        ...
        for(int i = 0; i < mdl.getElement(0).getStripCount(); i++){
            offset += wf.writeVector(mdl.getElement(0).getStrip(i).sphere.x, mdl.getElement(0).getStrip(i).sphere.y, mdl.getElement(0).getStrip(i).sphere.z, mdl.getElement(0).getStrip(i).sphere.w);
        }
    ...
    private void compressAndSave(byte[] dataBuffer){
        ...
        wrf.writeInt(0x0000006E);

        mdl.flags = getFlags(sysSize, graphicSize);
        wrf.writeInt(getFlags(sysSize, graphicSize));
        wrf.writeShort(0xDA78);

        System.out.println("Starting compression");
        wrf.writeArray(compress(dataBuffer));
    }
