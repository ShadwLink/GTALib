...
public class Element {
    ...
    public void attachTXD(String[] texNames, int[] texID){
        for(int i = 0; i < shaders.size(); i++){
            //file_io.Message.displayMsgLow("Started Shader " + i + " name is " + shaders.get(i).getTextureName());
            for(int j = 0; j < texNames.length; j++){
                //file_io.Message.displayMsgLow("texName" + j + ": " + texNames[j]);
                if(texNames[j] != null){
                    if(shaders.get(i).getTextureName().equalsIgnoreCase(texNames[j])){
                        ...
                        //file_io.Message.displayMsgLow("Found " + j);
    ...
    /**
     * Reset all values of this element:
     * Vertices
     * Strips
     * Shaders
     * hasVertexColors
    ...
    public void createModelPoly(int a, int b, int c, int stripIndex, boolean check) {
        //file_io.Message.displayMsgLow("Original poly: " + a + ", " + b + ", " + c);
        if(check){
        ...
        //file_io.Message.displayMsgLow("Created poly: " + a + ", " + b + ", " + c);
    ...
    /**
     * Check if the vertex is out of the current bounds if so it will set the bounds of this element to the current vertex
     * @param vertex Vertex to check this with
     */
    public void checkBounds(Vertex vertex){
        if(vertex.getVertexX() > max.getVertexX())  max.setVertexX(vertex.getVertexX());
        if(vertex.getVertexY() > max.getVertexY())  max.setVertexY(vertex.getVertexY());
        if(vertex.getVertexZ() > max.getVertexZ())  max.setVertexZ(vertex.getVertexZ());
        if(vertex.getVertexX() < min.getVertexX())  min.setVertexX(vertex.getVertexX());
        if(vertex.getVertexY() < min.getVertexY())  min.setVertexY(vertex.getVertexY());
        if(vertex.getVertexZ() < min.getVertexZ())  min.setVertexZ(vertex.getVertexZ());
    }
    ...
    public void displayBounds(){
        /*file_io.Message.displayMsgLow("Max: ");
        file_io.Message.displayMsgLow(max.getVertexX() + ", " + max.getVertexY() + ", " + max.getVertexZ());
        file_io.Message.displayMsgLow("Min: ");
        file_io.Message.displayMsgLow(min.getVertexX() + ", " + min.getVertexY() + ", " + min.getVertexZ());
        file_io.Message.displayMsgLow("Center: ");
        file_io.Message.displayMsgLow(center.getVertexX() + ", " + center.getVertexY() + ", " + center.getVertexZ());*/
    }
    ...
    public void render(GL2 gl){
        if(visible){
            for(int i = 0; i < strips.size(); i++){
                if(shaders.size() > i) gl.glBindTexture(gl.GL_TEXTURE_2D, shaders.get(strips.get(i).getShaderNumber()).getGLTex());
