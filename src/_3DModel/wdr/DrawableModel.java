...
public class DrawableModel {
    ...
    public void readSystemMemory(ByteReader br){
        ...
        if (shaderGroupOffset != -1) {
            Message.displayMsgHigh("Setting shader offset " + Util.getHexString(shaderGroupOffset));
    ...
    public String[] getDataValues(){
        ...
        int i = 0;
        values[i] = Util.getHexString(shaderGroupOffset);
        i++;
        values[i] = Util.getHexString(skeletonOffset);
        i++;
        ...
        for(int i2 = 0; i2 < levelOfDetailCount; i2++){
            values[i] = Util.getHexString(modelOffsets[i2]);
            i++;
        }
