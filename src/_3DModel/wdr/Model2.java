...
import _3DModel.Collections.PtrCollection;
import _3DModel.Collections.SimpleArray;
...
public class Model2 {
    ...
    public String[] getDataNames(){
        ...
        int i = 0;
        names[i] = "VTable"; i++;
        names[i] = "unknownVectorOffsets"; i++;
        names[i] = "materialMappingOffset"; i++;
        names[i] = "Unknown1"; i++; // the four following really should be bytes
        names[i] = "Unknown2"; i++;
        names[i] = "Unknown3"; i++;
        names[i] = "GeoCount"; i++;
        names[i] = "[UnknownVectors]"; i++;
        for(int i2 = 0; i2 < UnknownVectors.Count; i2++){
            names[i] = "  Vector " + (i2+1); i++;
        }
        names[i] = "[ShaderMappings]"; i++;
        for(int i2 = 0; i2 < ShaderMappings.Count; i2++){
            names[i] = "  ShaderMapping " + (i2+1); i++;
        }
...
//        }
    ...
    public String[] getDataValues(){
        ...
        int i = 0;
        values[i] = "" + VTable; i++;
        values[i] = Util.getHexString(unknownVectorOffsets); i++;
        values[i] = Util.getHexString(materialMappingOffset); i++;
        values[i] = "" + Unknown1; i++; // the four following really should be bytes
        values[i] = "" + Unknown2; i++;
        values[i] = "" + Unknown3; i++;
        values[i] = "" + GeoCount; i++;
        values[i] = ""; i++;
        for(int i2 = 0; i2 < UnknownVectors.Count; i2++){
            values[i] = "" + UnknownVectors.Values.get(i2); i++;
        }
        values[i] = ""; i++;
        for(int i2 = 0; i2 < ShaderMappings.Count; i2++){
            values[i] = "" + ShaderMappings.Values.get(i2); i++;
        }
