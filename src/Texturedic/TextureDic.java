...
public class TextureDic {
   ...
   public TextureDic(String fileName, ByteReader br, int gameType, GL2 gl, int fileSize) {
   ...
   public TextureDic(String fileName, ByteReader br, int gameType, GL2 gl, boolean compressed, int sysSize) {
        this.fileName = fileName;
        this.gameType = gameType;
        this.gl = gl;
        this.br = br;
    ...
    private boolean loadTextureDic(){
        switch(gameType){
            case 3:
                if(compressed) textureId = new TextureDic_IV().loadTextureDic(this, gl, compressed, 0);
                else textureId = new TextureDic_IV().loadTextureDic(this, gl, compressed, fileSize);
            break;
            default:
                textureId = new TextureDic_III_ERA().loadTextureDic(this, gl);
        }
        return true;
    }
    ...
    public void convertToWTD(WriteFunctions wrf){
        ...
        for(int i = 0; i < textures.size(); i++){
            offset += wf.writeInt((int)Util.genHash(textures.get(i).difTexName));
        }
        offset = addPaddingToRow(wf, offset);
        ...
        for(int i = 0; i < textureCount; i++){
            ...
            offset += wf.writeFloat(0);
            offset += wf.writeFloat(0);
            if(i < textureCount) offset += wf.writeOffset(offset + 0xF); //offset to next texturelist
            offset += wf.writeInt(0); //unknown
        ...
        for(int i = 0; i < textureCount; i++){
            System.out.println("Offset " + Util.getHexString(offset));
        ...
        wrf.writeInt(0x05435352);
        wrf.writeInt(0x00000008);
        ...
        wrf.writeArray(compress(wf.getArray()));
    }
    ...
    public void displayTextures(){
         for(int i = 0; i < textures.size(); i++){
            System.out.println("Texture" + i + " size " + Util.getHexString(textures.get(i).dataSize) + " offset " + Util.getHexString(textures.get(i).conversionOffset));
        }
    }
    ...
    public int getFlags(int sysSegSize, int gpuSegSize){
        int result = (getCompactSize(sysSegSize) & 0x7FFF)
            | (getCompactSize(gpuSegSize) & 0x7FFF) << 15
            | 3 << 30;
        return result;
    }
