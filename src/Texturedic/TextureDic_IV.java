...
public class TextureDic_IV {
	...
	public int[] loadTextureDic(TextureDic wtd, GL2 gl, boolean compressed, int sysSize) {
	...
	public void read(ByteReader br, GL2 gl) {
		...
		if (texCount > 0) {
			...
			for (int i = 0; i < pTexCount; i++) {
				...
				// Util.getHexString(texNameOffset));
				...
				// Util.getHexString(nextTexOffset));
				...
				// Util.getHexString(dataOffset+sysSize));
				...
				if (gl != null) {
					...
					if (compression.equals("DXT1")) {
						...
						gl.glCompressedTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_COMPRESSED_RGB_S3TC_DXT1_EXT, width,
								height, 0, dataSize, br.getByteBuffer(dataSize));
					} else if (compression.equals("DXT3")) {
						int dataSize = (int) (width * height);
						System.out.println("DXT3 Datasize: " + dataSize);
						gl.glCompressedTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_COMPRESSED_RGBA_S3TC_DXT3_EXT, width,
								height, 0, dataSize, br.getByteBuffer(dataSize));
					} else if (compression.equals("DXT5")) {
						int dataSize = (int) (width * height);
						System.out.println("DXT5 Datasize: " + dataSize);
						gl.glCompressedTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_COMPRESSED_RGBA_S3TC_DXT5_EXT, width,
								height, 0, dataSize, br.getByteBuffer(dataSize));
					}
