...
public class Strip {
    ...
    public int addVertex(Vertex vertex){
        ...
        if(!vert.contains(vertex)){
            ret = vert.size();
            vert.add(vertex);
            //file_io.Message.displayMsgLow("Added vertex at " + ret);
        }else{
            ret = vert.indexOf(vertex);
            //file_io.Message.displayMsgLow("Vertex bestond al in deze strip " + ret);
        }
        return ret;
    }
    ...
    private float getMax(){
        float value = max.x;
        if(value < max.y) value = max.y;
        if(value < max.z) value = max.z;
        if(value < 0-min.x) value = 0-min.x;
        if(value < 0-min.y) value = 0-min.y;
        if(value < 0-min.z) value = 0-min.z;
